...
{phang}
{cmd:create_init_csv} Generates an initial CSV file (`init.csv`) specifying the silo names, start times, end times, and treatment times.
...
{pstd}
{cmd:create_init_csv} silo_names(string) start_times(string) end_times(string) treatment_times(string)[{it:covariates(string)} {it:filename(string)} {it:filepath(string)}]{p_end}
